...
# 🏗️ DASHBOARD HAUPT-ARCHITEKTUR

## Die 3 Haupt-Modi (Chart-Ansichten)
...
│                                                                      │
│  ┌─────────────────┐                                                │
│  │   1 Bot-Type    │ ──► MAINCHART (Golden State)                   │
│  │   ausgewählt    │     • Start + End Werte                        │
│  └─────────────────┘     • Flüssiger Graph                          │
│                          • Alle Features vollständig                 │
│                                                                      │
│  ┌─────────────────┐     ┌──────────────────────────────┐          │
│  │  2+ Bot-Types   │     │  Toggle: Compare vs Added    │          │
│  │   ausgewählt    │ ──► └──────────────────────────────┘          │
│  └─────────────────┘              │                                 │
│                                   ▼                                 │
│                    ┌──────────────┴──────────────┐                 │
...
│                    ▼                             ▼                  │
│         ┌──────────────────┐          ┌──────────────────┐         │
│         │   COMPARE MODE   │          │   EDIT (ADDED)   │         │
│         │  (Golden State)  │          │   MODE           │         │
│         │                  │          │                  │         │
│         │ • Start + End    │          │ • NUR End Werte  │         │
│         │ • Flüssiger Graph│          │ • NICHT flüssig  │         │
│         │ • Farbcodiert    │          │ • Datenpunkte    │         │
│         └──────────────────┘          └────────┬─────────┘         │
│                                                │                    │
│                                                ▼                    │
│                                   ┌────────────────────────┐       │
│                                   │ Sub-Toggle im Update-  │       │
│                                   │ Verlauf Header:        │       │
│                                   │                        │       │
│                                   │  [Analysis] [Overlay]  │       │
│                                   └────────────────────────┘       │
...
## Warum Analysis/Overlay im Edit-Modus?

| Modus | Werte | Graph-Typ | Problem |
|-------|-------|-----------|---------|
| MainChart | Start + End | Flüssig | Kein Problem |
| Compare | Start + End | Flüssig | Kein Problem |
| **Edit (Added)** | **Nur End** | **Nicht flüssig** | Kann nicht alle Metriken sauber darstellen |

**Lösung für Edit-Modus:**
- **Analysis**: Zeigt Metriken korrekt an (einzelne Betrachtung)
- **Overlay**: Kommt später (alternative Darstellung)

## State-Variablen für Modi-Erkennung

```typescript
// 1 Bot-Type → MainChart
selectedChartBotTypes.length === 1

// 2+ Bot-Types → Compare ODER Edit
selectedChartBotTypes.length >= 2

// Compare vs Edit Toggle
alleEintraegeMode: 'compare' | 'added'

// Edit-Modus Sub-Toggle
addedModeView: 'analysis' | 'overlay'

// Stift-Modus aktiv
markerEditActive: boolean

// Analyze-Status aktiv (nach Stift + Apply + Analyze Button)
analyzeMode: boolean
analyzeModeBounds: { startTs, endTs } | null
```
